...
use crate::{Environment, StoreContext};
...
impl ChainsManager {
    pub fn new() -> Self {
        ...
        let mainnet = Chain::new(MAINNET_PARAMS, MAINNET_CHECKPOINT_ARRAY, &authentication_manager,NetworkContext::new(), StoreContext::new());
        let testnet = Chain::new(TESTNET_PARAMS, TESTNET_CHECKPOINT_ARRAY, &authentication_manager, NetworkContext::new(), StoreContext::new());
    ...
    pub fn create_devnet(&mut self, r#type: DevnetType) -> Chain {
        ...
        let second_checkpoint = Checkpoint::create_dev_net_genesis_block_checkpoint_for_parent_checkpoint(genesis_checkpoint, r#type, params.protocol_version.clone());
        Chain::new(
            params,
    ...
    pub fn remove_devnet(&mut self, chain: &mut Chain) {
        if let ChainType::DevNet(devnet_type) = chain {
            ...
            let key = DEVNET_CHAINS_KEY.to_string();
            let classes = vec!["String".to_string(), "Array".to_string(), "Checkpoint".to_string()];
            let mut registered_devnets_dictionary = Keychain::get_dict::<String, KeychainDictValueKind>(key, classes).unwrap_or(HashMap::new());
            if registered_devnets_dictionary.keys().contains(&devnet_type.identifier()) {
                registered_devnets_dictionary.remove(&devnet_type.identifier());
                Keychain::set_dict::<String, KeychainDictValueKind>(registered_devnets_dictionary, DEVNET_CHAINS_KEY.to_string(), false)
            }
    ...
    pub fn wipe_peer_data_for_chain(&self, mut chain: &Chain, context: &ManagedContext) {
        ...
        match PeerEntity::delete_all_peers_for_chain(chain.params.chain_type, context) {
            Ok(deleted) => println!("All peer entities for chain {:?} are deleted", chain.params.chain_type),
    ...
    pub fn wipe_blockchain_data_for_chain(&self, mut chain: &mut Chain, context: &ManagedContext) {
        self.stop_sync_for_chain(&mut chain);
        match ChainEntity::wipe_blockchain_data(chain.params.chain_type, context) {
            Ok(deleted) => {
            ...
            Err(err) => println!("Error deleting chain entity {:?}", chain.params.chain_type)
        }
    }

    pub fn wipe_blockchain_non_terminal_data_for_chain(&self, mut chain: &mut Chain, context: &ManagedContext) {
        self.stop_sync_for_chain(&mut chain);
        match ChainEntity::wipe_blockchain_non_terminal_data(chain.params.chain_type, context) {
            Ok(deleted) => {
    ...
    pub fn wipe_masternode_data_for_chain(&self, mut chain: &mut Chain, context: &ManagedContext) {
        self.stop_sync_for_chain(&mut chain);
        match ChainEntity::get_chain(chain.params.chain_type, context) {
            Ok(entity) => {
            ...
            Err(err) => println!("Error retrieving chain entity {:?}", chain.params.chain_type)
        }
    }
    ...
    pub fn wipe_spork_data_for_chain(&self, mut chain: &mut Chain, context: &ManagedContext) {
        ...
        match ChainEntity::get_chain(chain.params.chain_type, context) {
            Ok(entity) => {
                ...
                UserDefaults::remove_object_for_key(format!("{}_{}", chain.unique_id, LAST_SYNCED_MASTERNODE_LIST).as_str());
                DispatchContext::main_context().queue(|| {
                    NotificationCenter::post(Notification::SporkListDidUpdate(chain));
                })
            },
            Err(err) => println!("Error retrieving chain entity {:?}", chain.params.chain_type)
        }
    ...
    pub fn wipe_wallet_data_for_chain(&self, mut chain: &mut Chain, force_reauth: bool, context: &ManagedContext) {
        self.stop_sync_for_chain(&mut chain);
        match ChainEntity::wipe_wallet_data(chain.params.chain_type, context) {
            Ok(deleted) => {
                chain.wipe_blockchain_info(context);
                chain.restart_chain_sync_start_height();
                chain.restart_chain_terminal_sync_start_height();
        ...
        if !force_reauth && self.authentication_manager.did_authenticate {
            ...
            match self.authentication_manager.authenticate_with_prompt(
                ...
                Err(err) => println!("Error authenticating {:?}", chain.params.chain_type)
            }
        }
    }
    ...
    pub fn db_size(&self) -> u64 {
        ...
        if let Some(meta) = fs::metadata(store_url) {
            meta.len()
        } else {
            0
        }
