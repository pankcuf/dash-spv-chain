    ...
    fn encode(&self) -> Self::Item {
        ...
        let previous_entry_hashes = boxed_vec(
            self.previous_entry_hashes
                .iter()
                .map(|(&common::BlockData { hash: block_hash, height: block_height }, &hash)| types::MasternodeEntryHash { block_hash: block_hash.0, block_height, hash: hash.0 })
                .collect(),
        );
...
impl ToFFI for common::BlockData {
