...
#[derive(Copy, Clone, PartialEq, Eq, PartialOrd, Ord, Hash, FromSqlRow, AsExpression)]
...
macro_rules! impl_bytes_decodable_lt {
    ($var_type: ident) => {
        #[proc_macro_derive(BytesDecodable, attributes())]
...
macro_rules! define_serde {
    ($uint_type: ident) => {
        impl serde::Serialize for $uint_type {
            fn serialize<S: serde::Serializer>(&self, serializer: S) -> std::result::Result<S::Ok, S::Error> {
                serializer.serialize_str(self.0.to_hex().as_str())
            }
        }
        impl<'de> serde::Deserialize<'de> for $uint_type {
            fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> std::result::Result<Self, D::Error> {
                let s = String::deserialize(deserializer)?;
                $uint_type::from_hex(s.as_str()).map_err(serde::de::Error::custom)
            }
        }
    }
}
...
macro_rules! define_bytes_to_big_uint {
    ($uint_type: ident, $byte_len: expr) => {
        ...
        // impl diesel::Expression for $uint_type {
        //     type SqlType = sql_types::Binary;
        // }
        // impl<DB> ToSql<sql_types::Binary, DB> for $uint_type where DB: Backend, [u8]: ToSql<sql_types::Binary, DB> {
        //     fn to_sql<W: Write>(&self, out: &mut Output<W, DB>) -> serialize::Result {
        //         self.as_bytes().to_sql(out)
        //     }
        // }
        // impl FromSql<sql_types::Binary, Sqlite> for $uint_type {
        //     fn from_sql(bytes: *const u8) -> deserialize::Result<Self> {
        //         $uint_type::from_const(bytes).ok_or(Box::try_from(util::Error::Default(&format!(""))).unwrap())
        //     }
        // }
        // impl<'a> FromSql<sql_types::Binary, Sqlite> for dyn ConstDecodable<'a, $uint_type> {
        //     fn from_sql(bytes: *const u8) -> deserialize::Result<Self> {
        //         Self::from_const(bytes).ok_or(Box::try_from(util::Error::Default(&format!(""))).unwrap())
        //     }
        // }
...
define_serde!(UInt128);
// TODO: maybe need to deal with base58
define_serde!(UInt160);
define_serde!(UInt256);
define_serde!(UInt384);
define_serde!(UInt512);
define_serde!(UInt768);
...
impl UInt128 {
    pub fn ip_address_from_i32(value: i32) -> Self {
        ...
        0u64.enc(writer);
        0xffffi32.swap_bytes().enc(writer);
        value.swap_bytes().enc(writer);
...
impl UInt256 {
    ...
    pub fn get_compact_le(&self) -> i32 {
        ...
        if size <= 3 {
            compact = u32::from_le_bytes(clone_into_array(&self.0[..4])) << 8 * (3 - size);
        } else {
            let bn = self.shift_right_le((8 * (size - 3)) as u8);
            compact = u32::from_le_bytes(clone_into_array(&bn.0[..4]));
        }
...
// impl diesel::Expression for dyn AsBytes {
//     type SqlType = Binary;
// }
//
//
