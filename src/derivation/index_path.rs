...
use crate::crypto::byte_util::{AsBytesVec, clone_into_array};
...
pub trait IIndexPath: Sized {
    type Item: Copy + Clone + Display + Debug + IndexHardSoft + PartialEq;
    ...
    fn index_path_by_removing_first_index(&self) -> IndexPath<Self::Item> {
        IndexPath::index_path_with_indexes(if self.indexes().len() <= 1 {
            vec![]
        } else {
...
impl<T> IIndexPath for IndexPath<T> where T: Copy + Debug + Display + IndexHardSoft + PartialEq {
    type Item = T;
