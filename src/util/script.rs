...
mod tests {
    ...
    fn check_address_with_script_pubkey(data: &[u8], script: &ScriptMap, address: Option<String>) {
        let elem = data.to_vec().script_elements();
        let addr = Address::with_script_pub_key(&data.to_vec(), script);
        assert_eq!(addr, address, "Address with script pubkey don't match")
    }

    fn check_address_with_script_signature(data: &[u8], script: &ScriptMap, address: Option<String>) {
        let elem = data.to_vec().script_elements();
        let addr = Address::with_script_sig(&data.to_vec(), script);
