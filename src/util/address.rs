pub mod Address {
    ...
    pub fn from_hash160_for_script_map(hash: &UInt160, script: &ScriptMap) -> String {
        let mut writer: Vec<u8> = Vec::new();
        script.pubkey.enc(&mut writer);
        hash.enc(&mut writer);
    ...
    pub fn is_valid_dash_address_for_script_map(address: &String, script: &ScriptMap) -> bool {
        if address.len() > 35 {
            return false;
        }
        match base58::from_check(address.as_str()) {
            Ok(d) if d.len() == 21 => d[0] == script.pubkey || d[0] == script.script,
            _ => false
        }
    }
