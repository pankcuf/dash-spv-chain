...
pub struct SpecialTransactionWalletHolder {
    ...
    provider_registration_transactions: HashMap<UInt256, &'static ProviderRegistrationTransaction>,
    provider_update_service_transactions: HashMap<UInt256, &'static ProviderUpdateServiceTransaction>,
    provider_update_registrar_transactions: HashMap<UInt256, &'static ProviderUpdateRegistrarTransaction>,
    provider_update_revocation_transactions: HashMap<UInt256, &'static ProviderUpdateRevocationTransaction>,
    credit_funding_transactions: HashMap<UInt256, &'static CreditFundingTransaction>,
    transactions_to_save: Vec<&'static dyn ITransaction>,
    transactions_to_save_in_block_save: HashMap<u32, Vec<&'static dyn ITransaction>>,
...
impl SpecialTransactionWalletHolder {
    ...
    pub fn derivation_paths(&self) -> Vec<Box<dyn IDerivationPath>> {
    ...
    pub fn persist_incoming_transactions_attributes_for_block_save_with_number(&mut self, block_number: u32, context: &ManagedContext) {
        if let Some(transactions) = self.transactions_to_save_in_block_save.get(&block_number) {
            transactions.iter().for_each(|mut transaction| {
                transaction.set_initial_persistent_attributes_in_context(context);
            });
        }
    ...
    pub fn credit_funding_transaction_for_blockchain_identity_unique_id(&self, unique_id: &UInt256) -> Option<&CreditFundingTransaction> {
        if let Some(&tx) = self.credit_funding_transactions.get(unique_id) {
            Some(tx)
        } else {
            None
        }
