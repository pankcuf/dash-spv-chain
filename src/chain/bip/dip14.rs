...
//
fn _ckd_priv(mut k: UInt256, mut c: UInt256, key: &[u8]) {
    // I = HMAC-SHA512(c, k|P(k) || i)
    // k = IL + k (mod n)
    // c = IR
    let i = UInt512::hmac(key, &c.0);
    c = UInt256(clone_into_array(&i.0[..32]));
    secp256k1_mod_add(&mut k, &c);
}


pub fn ckd_priv(mut k: UInt256, mut c: UInt256, i: u32) {
    let mut buf = &mut [0u8; 65];
    if i & BIP32_HARD != 0 {
...
pub fn ckd_priv_256(mut k: UInt256, mut c: UInt256, i: &UInt256, hardened: bool) {
    ...
    if hardened {
        ...
        secp256k1_point_gen(&k).enc(writer);
    };
    if i_is_31_bits {
        i.u32_le().swap_bytes().enc(writer);
    } else {
        i.enc(writer);
    };
    _ckd_priv(k, c, writer.as_slice());
}
...
//
fn _ckd_pub(mut k: ECPoint, mut c: UInt256, key: &[u8]) {
    // I = HMAC-SHA512(c, P(K) || i)
    let i = UInt512::hmac(key, &c.0);
    // c = IR
    c = UInt256(clone_into_array(&i.0[..32]));
    // K = P(IL) + K
    k = secp256k1_point_add(&k, &c);
}

pub fn ckd_pub(mut k: ECPoint, mut c: UInt256, i: u32) {
    if i & BIP32_HARD != 0 {
...
pub fn ckd_pub_256(mut k: ECPoint, mut c: UInt256, i: UInt256, hardened: bool) {
    if hardened {
        // can't derive private child key from public parent key
        return;
    }
