...
use crate::util::crypto::ceil_log2;
...
impl MerkleTree {
    ...
    pub fn merkle_root(&self) -> Option<UInt256> {
            ...
            |left, right|
                match (left, right) {
                    (WalkHashResult::Hash(Some(l)), WalkHashResult::Hash(r_opt)) =>
                        ...
                        // Some(self.hash_branch(l, r_opt.unwrap_or(l))),
                    _ => WalkHashResult::Hash(None)
                }) {
        ...
        //     0,
        //     |hash, _flag| WalkHashResult::Hash(hash),
        //     |left, right| Some(self.hash_branch(left, right.unwrap_or(left)))
        // )
    }
