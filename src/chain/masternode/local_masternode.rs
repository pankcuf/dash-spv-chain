...
use crate::chain::masternode::local_masternode_status::LocalMasternodeStatus;
...
use crate::util::{address_with_script_pub_key, UTXO};
...
use crate::storage::models::masternode::{LocalMasternodeEntity, NewLocalMasternodeEntity};
...
pub struct LocalMasternode {
    ...
    pub provider_update_registrar_transactions: Vec<ProviderUpdateRegistrarTransaction>,
    pub provider_update_service_transactions: Vec<ProviderUpdateServiceTransaction>,
    pub provider_update_revocation_transactions: Vec<ProviderUpdateRevocationTransaction>,
}
...
impl LocalMasternode {
    ...
    pub fn init_with_provider_registration_transaction(transaction: &ProviderRegistrationTransaction) -> Self {
        ...
        Self {
            ...
            ip_addres: transaction.ip_address,
    ...
    pub fn register_in_associated_wallets(&self) {
        self.operator_keys_wallet?.register_masternode_operator(self);
        self.owner_keys_wallet?.register_masternode_owner(self);
        self.voting_keys_wallet?.register_masternode_voter(self);
    }

    pub fn force_operator_public_key(&self, operator_public_key: &BLSKey) -> bool {
    ...
    pub fn force_owner_private_key(&self, owner_private_key: &ECDSAKey) -> bool {
        if let Some(mut wallet) = &self.owner_keys_wallet {
    ...
    pub fn force_voting_key(&self, voting_key: &ECDSAKey) -> bool {
        if let Some(mut wallet) = &self.owner_keys_wallet {
    ...
    pub fn operator_payout_address(&self) -> Option<String> {
        if let Some(tx) = &self.provider_registration_transaction {
            if !self.provider_update_service_transactions.is_empty() {
                address_with_script_pub_key(self.provider_update_service_transactions.last().unwrap().script_payout(), tx.chain)
            }
        }
        None
    }
    ...
    pub fn operator_key_from_seed(&self, seed: Vec<u8>) -> Option<BLSKey> {
        if let Some(wallet) = &self.operator_keys_wallet {
            if let Some(tx) = &self.provider_registration_transaction {
                ...
                return path.private_key_for_hash160(hash, seed)
            }
        }
        None
    }
    ...
    pub fn operator_key_string_from_seed(&self, seed: Vec<u8>) -> Option<String> {
        if let Some(key) = self.operator_key_from_seed(seed) {
        ...
        if let Some(wallet) = &self.owner_keys_wallet {
            if let Some(seed) = seed {
                if let Some(pro_reg_tx) = self.provider_registration_transaction {
                    let path = AuthenticationKeysDerivationPath::provider_owner_keys_derivation_path_for_wallet(wallet);
                    return path.private_key_for_hash160(pro_reg_tx.owner_key_hash, seed)
                }
            }
        }
        None
    ...
    pub fn owner_key_string_from_seed(&self, seed: Option<Vec<u8>>) -> Option<String> {
        if let Some(key) = self.owner_key_from_seed(seed) {
            key.secret_key_string()
        } else {
            None
        }
        ...
        if let Some(wallet) = &self.voting_keys_wallet {
            if let Some(seed) = seed {
                if let Some(pro_reg_tx) = self.provider_registration_transaction {
                    ...
                    return path.private_key_for_hash160(pro_reg_tx.voting_key_hash, seed);
                }
            }
        }
        None
    ...
    pub fn owner_public_key_data(&self) -> Vec<u8> {
        if let Some(wallet) = &self.owner_keys_wallet {
            if let Some(tx) = &self.provider_registration_transaction {
                let path = AuthenticationKeysDerivationPath::provider_owner_keys_derivation_path_for_wallet(wallet);
                return path.public_key_data_for_hash160(tx.owner_key_hash);
            }
        }
        vec![]
    }
    ...
    pub fn operator_public_key_data(&self) -> Vec<u8> {
        if let Some(wallet) = &self.operator_keys_wallet {
            if let Some(tx) = &self.provider_registration_transaction {
                let path = AuthenticationKeysDerivationPath::provider_operator_keys_derivation_path_for_wallet(wallet);
                let hash = UInt160::hash160(&tx.operator_key.0);
                return path.public_key_data_for_hash160(hash);
            }
        }
        vec![]
    }
    ...
    pub fn voting_public_key_data(&self) -> Vec<u8> {
        if let Some(wallet) = &self.voting_keys_wallet {
            if let Some(tx) = &self.provider_registration_transaction {
                let path = AuthenticationKeysDerivationPath::provider_voting_keys_derivation_path_for_wallet(wallet);
                return path.public_key_data_for_hash160(tx.voting_key_hash);
            }
        }
        vec![]
    }
    ...
    pub fn voting_key_string_from_seed(&self, seed: Option<Vec<u8>>) -> Option<String> {
        if let Some(key) = self.voting_key_from_seed(seed) {
            key.secret_key_string()
        } else {
            None
        }
    ...
    fn associate_name(&mut self) {
        if let Some(data) = Keychain::get_string(self.masternode_identifier_for_name_storage()) {
            self.name = data
        }
    }
    ...
    pub fn register_name(&mut self, name: String) {
        if name != self.name {
            let _ = Keychain::set_string(name, self.masternode_identifier_for_name_storage(), false);
    ...
    pub fn update_with_update_registrar_transaction(&mut self, transaction: ProviderUpdateRegistrarTransaction, save: bool) {
        ...
        if let Some(wallet) = &self.operatorKeysWallet {
            let path = AuthenticationKeysDerivationPath::provider_operator_keys_derivation_path_for_wallet(wallet);
            if let Some(index) = path.index_of_known_address(transaction.operator_address()) {
        ...
        if let Some(wallet) = &self.voting_keys_wallet {
            let path = AuthenticationKeysDerivationPath::provider_voting_keys_derivation_path_for_wallet(wallet);
            if let Some(index) = path.index_of_known_address(transaction.voting_address()) {
    ...
    pub fn update_with_update_service_transaction(&mut self, transaction: ProviderUpdateServiceTransaction, save: bool) {
        if !self.provider_update_service_transactions.contains(&transaction) {
            ...
            self.ip_addres = transaction.ip_address.clone();
...
impl EntityConvertible for LocalMasternode {
    fn to_entity<T, U>(&self) -> U
        where
            T: Table,
            diesel::query_source::FromClause: QueryFragment<Sqlite>, U: Insertable<T>,
            diesel::insertable::Values: IValues {
