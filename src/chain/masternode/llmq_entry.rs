...
impl PartialEq for LLMQEntry {
    fn eq(&self, other: &Self) -> bool {
        self == other || self.entry_hash == other.entry_hash
    }
}

impl std::hash::Hash for LLMQEntry {
    fn hash<H: Hasher>(&self, state: &mut H) {
        state.write(self.entry_hash.as_bytes());
    }
}
...
impl LLMQEntry {
    ...
    pub fn llmq_quorum_hash(&self) -> UInt256 {
        // TODO: check varint.enc vs u8.enc ?? capacity
        let mut buffer: Vec<u8> = Vec::with_capacity(33);
        VarInt(self.llmq_type as u64).enc(&mut buffer);
    ...
    pub fn merge_with_llmq_entry(&mut self, entry: &LLMQEntry) {
        self.all_commitment_aggregated_signature = entry.all_commitment_aggregated_signature;
        self.commitment_hash = entry.commitment_hash;
        self.llmq_type = entry.llmq_type;
        self.entry_hash = entry.entry_hash;
        self.llmq_hash = entry.llmq_hash;
        self.public_key = entry.public_key;
        self.threshold_signature = entry.threshold_signature;
        self.verification_vector_hash = entry.verification_vector_hash;
        self.index = entry.index;
        self.saved = entry.saved;
        self.signers_bitset = entry.signers_bitset.clone();
        self.signers_count = entry.signers_count;
        self.valid_members_bitset = entry.valid_members_bitset.clone();
